...
cat(paste("\nmerge_sequences.R started at ", Sys.time()), file = progress_file, append = TRUE)
...
dadaRs <- readRDS(paste(output_path, 'temp', 'dadaRs.rds', sep = '/'))

# merge inferred forward and reverse sequences ####
mergers <- mergePairs(dadaFs, filtFs, dadaRs, filtRs)
saveRDS(mergers, paste(output_path, 'temp', 'mergers.rds', sep = '/'))
cat(paste("\nmerge_sequences.R ended at ", Sys.time()), file = progress_file, append = TRUE)
