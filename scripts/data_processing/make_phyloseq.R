...
cat(paste("\nmake_phyloseq.R started at ", Sys.time()), file = progress_file, append = TRUE)
...
taxtab <- readRDS(paste(output_path, 'temp', 'taxtab.rds', sep = '/')) 
seqtab <- readRDS(paste(output_path, 'temp', 'seqtab.rds', sep = '/'))
tree <- read_tree(paste(output_path, 'temp', 'fasttree.tree', sep = '/'))
...
saveRDS(ps, paste(output_path, 'ps.rds', sep = '/'))
save(ps, file = paste(output_path, 'ps.Rdata', sep = '/'))

cat(paste("\nmake_phyloseq.R ended at ", Sys.time()), file = progress_file, append = TRUE)
