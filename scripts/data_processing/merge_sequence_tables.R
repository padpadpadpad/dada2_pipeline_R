...
cat(paste("\nmerge_sequence_tables.R started at ", Sys.time()), file = progress_file, append = TRUE)

seqtabs <- trimws(unlist(strsplit(subset(config, input == 'seqtabs')$value, ',')))

seqtab <- dada2::mergeSequenceTables(tables = seqtabs,
  repeats = "error",
  orderBy = "abundance",
  tryRC = TRUE
)
...
saveRDS(seqtab, paste(output_path, 'temp', 'seqtab_all.rds', sep = '/'))

write.csv(config, paste(file.path(output_path, 'config.csv')), row.names = FALSE, quote = FALSE)

cat(paste('\nmerge_sequence_tables.R ended at', Sys.time()), file = progress_file, append = TRUE)