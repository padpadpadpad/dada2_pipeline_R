...
cat(paste("\nestimate_errors_mod1.R started at ", Sys.time()), file = progress_file, append = TRUE)
...
cat(
  paste('\nForward error rates completed at', Sys.time()),
  file = progress_file,
  append = TRUE
)
# Learn reverse error rates
dd_learnR <- learnErrors(
  filtRs,
  errorEstimationFunction = loessErrfun_mod,
  multithread = TRUE,
  randomize = TRUE,
...
cat(
  paste('\nReverse error rates completed at', Sys.time()),
  file = progress_file,
  append = TRUE
)
# save out error rates
saveRDS(dd_learnF,
        paste(output_path, 'temp', 'fwd_error.rds', sep = '/'))
saveRDS(dd_learnR,
        paste(output_path, 'temp', 'rev_error.rds', sep = '/'))

# plot error rates ####
pdf(paste(plot_path, 'error_rates_mod.pdf', sep = '/'))
plotErrors(dd_learnF, nominalQ=TRUE) +
  ggtitle('Forward error rates')
plotErrors(dd_learnR, nominalQ=TRUE) +
  ggtitle('Reverse error rates')
dev.off()

cat(paste("\nestimate_errors_mod1.R ended at ", Sys.time()), file = progress_file, append = TRUE)
